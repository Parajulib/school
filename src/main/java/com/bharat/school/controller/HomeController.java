package com.bharat.school.controller;
...
import com.bharat.school.domain.Role;
...
public class HomeController {
			...
			)
	{
		...
		{
			model.addAttribute("username_error", "Username must be atleast 6 character or more");
			return "signup";
		}
		if(!username.matches("^[a-z]+$"))
		{
			model.addAttribute("username_error", "Invalid Characters ! username should be lower cased characters only.");
			return "signup";
		}
		...
		if(user != null)
		{
			model.addAttribute("username_error", "Username already exist. Please choose different user name.");
			return "signup";
		}
		
		if(password.length() < 6)
		{
			model.addAttribute("password_error", "Password must be atleast 6 character or more.");
			return "signup";
		}
		
		if(!password.equals(retypePassword))
		{
			model.addAttribute("retype_password_error", "Retype Password didn't match.");
			return "signup";
		}
		...
		if(userType.equals("student"))
		{
			...
			if(student == null)
			{
				model.addAttribute("regNumber_error", "Student with given registration number not found.");
				return "signup";
			}else if(! student.getDateOfBirth().equals(dateOfBirth)){
				model.addAttribute("dateOfBirth_error", "Date of Birth not valid.");
				return "signup";
			}else if(student.getUserLogin() != null){
				
				model.addAttribute("dateOfBirth_error", "You already have the user account created. Please try forget password to retrive.");
				return "signup";
				...
				session.setAttribute("notification", "User Login Created Successfully. Please login with your username and Password.");
